<template>
  <div class="max-w-full">
    ...
    <div class="mb-5 grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-3">
      ...
      <template v-else>
        <StatCard :icon="GraduationCap" label="Total Students" :value="store.totalStudents" color="blue" />
        <StatCard :icon="Layers" label="Generations" :value="store.generations.length" color="green" />
    ...
    <div class="rounded-[10px] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.08)]">
      ...
      <div v-if="loading" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div v-for="n in 4" :key="n" class="h-[150px] animate-pulse rounded-xl border border-slate-100 bg-slate-50" aria-hidden="true" />
      </div>

      <div v-else-if="filteredGenerations.length === 0" class="flex flex-col items-center justify-center gap-2 py-14 text-center">
      ...
      <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <DirectoryGenerationCard v-for="group in filteredGenerations" :key="group.generation ?? '__unassigned__'" :group="group" />
      </div>
    </div>
  </div>
</template>
...
const filteredGenerations = computed(() => filterGenerations(store.generations, search.value))
