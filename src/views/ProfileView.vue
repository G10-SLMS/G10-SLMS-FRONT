<template>
  <div class="max-w-[900px] px-8 py-6">
    <header class="mb-6 flex items-center justify-between">
      ...
      <RouterLink
        to="/profile/edit"
        class="inline-flex items-center gap-1.5 rounded-lg border-none bg-blue-600 px-4 py-2 text-sm font-semibold text-white no-underline hover:bg-blue-700"
      >
        <Pencil :size="16" />
        Edit Profile
      </RouterLink>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      <section class="rounded-xl border border-slate-200 bg-white p-5 px-6 md:col-span-2">
        ...
        <dl class="flex flex-col gap-3">
          <div class="flex items-center justify-between text-sm">
            <dt class="flex items-center gap-2 font-medium text-slate-500"><Mail :size="16" /> Email</dt>
            <dd class="font-semibold text-slate-900">{{ user?.email }}</dd>
          </div>
          <div class="flex items-center justify-between text-sm" v-if="user?.gender">
            <dt class="flex items-center gap-2 font-medium text-slate-500"><User :size="16" /> Gender</dt>
            <dd class="capitalize font-semibold text-slate-900">{{ user.gender }}</dd>
          </div>
          <div class="flex items-center justify-between text-sm">
            <dt class="flex items-center gap-2 font-medium text-slate-500"><Shield :size="16" /> Role</dt>
      ...
      <section class="rounded-xl border border-slate-200 bg-white p-5 px-6">
        ...
        <ProfileProviderRow name="Google" :linked="providerLinked('google')">
          <template #icon>
            <path fill="#EA4335" d="M12 11v2.4h6.7c-.3 1.6-2.1 4.6-6.7 4.6-4 0-7.3-3.3-7.3-7.4S8 3.2 12 3.2c2.3 0 3.8.9 4.7 1.8l3.2-3.1C17.9 0.2 15.2-.7 12-.7 5.6-.7.5 4.4.5 10.8s5.1 11.5 11.5 11.5c6.6 0 11-4.6 11-11.1 0-.7-.1-1.3-.2-1.9H12z" />
          </template>
        </ProfileProviderRow>

        <ProfileProviderRow name="GitHub" :linked="providerLinked('github')">
          <template #icon>
            <path fill="#181717" d="M12 .3a12 12 0 00-3.8 23.4c.6.1.8-.3.8-.6v-2.2c-3.3.7-4-1.6-4-1.6-.6-1.4-1.4-1.7-1.4-1.7-1.1-.8.1-.8.1-.8 1.2.1 1.9 1.3 1.9 1.3 1.1 1.9 2.9 1.3 3.5 1 .1-.8.4-1.3.8-1.6-2.7-.3-5.5-1.3-5.5-5.9 0-1.3.5-2.4 1.3-3.2-.1-.3-.6-1.6.1-3.2 0 0 1-.3 3.4 1.2a11.5 11.5 0 016 0c2.3-1.6 3.3-1.2 3.3-1.2.7 1.6.2 2.9.1 3.2.8.8 1.3 1.9 1.3 3.2 0 4.6-2.8 5.6-5.5 5.9.4.4.8 1.1.8 2.2v3.3c0 .3.2.7.8.6A12 12 0 0012 .3z" />
          </template>
        </ProfileProviderRow>
      </section>
