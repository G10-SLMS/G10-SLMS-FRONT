<template>
  <div>
    ...
    <ul class="divide-y divide-slate-100 sm:hidden">
      <li v-for="r in items" :key="r.id" class="flex flex-col gap-3 p-4">
            ...
            <template v-if="!auth.isStudent">
              ...
              <span
                ...
                :class="[
                  ...
                  getAvatarColor(r.user?.name ?? 'User'),
                ]"
              >
              ...
              <div class="flex flex-col">
                ...
                <p class="m-0 text-[12px] text-slate-500">
                  ID: {{ r.user?.student_id != null ? '0' + r.user.student_id : '—' }}
                </p>
              </div>
            </template>
            <template v-else>
              <div class="flex flex-col">
                <p class="m-0 truncate font-semibold text-slate-900">{{ r.leave_type_name }}</p>
                <p class="m-0 text-[12px] text-slate-500">Request #{{ r.id }}</p>
              </div>
            </template>
          </div>
        ...
        <p v-if="!auth.isStudent" class="m-0 text-[13px] font-semibold text-slate-900">
          {{ r.leave_type_name }}
        </p>
        ...
        <div class="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[13px]">
          ...
          <div>
            <span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400"
              >Duration</span
            >
            <p class="m-0 text-slate-600">{{ r.duration_label ?? `${r.total_days}d` }}</p>
          </div>
          <div>
            <span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400"
              >Submitted</span
      ...
      <table class="w-full min-w-[720px] border-collapse text-sm">
        <thead>
          <tr class="sticky top-0 z-10 bg-slate-50">
            <th
              v-if="!auth.isStudent"
              class="border-b border-slate-200 px-4 py-3.5 pl-5 text-left text-[11px] font-bold uppercase tracking-wide text-slate-500"
            >
              Student
            </th>
            <th
              v-if="!auth.isStudent"
              class="border-b border-slate-200 px-4 py-3.5 text-left text-[11px] font-bold uppercase tracking-wide text-slate-500"
            >
              Student ID
            </th>
            <th
            ...
            <th
              class="border-b border-slate-200 px-4 py-3.5 text-center text-[11px] font-bold uppercase tracking-wide text-slate-500"
            >
              Days
            </th>
            <th
            ...
            <th
              class="border-b border-slate-200 px-4 py-3.5 pr-5 text-right text-[11px] font-bold uppercase tracking-wide text-slate-500"
            >
              Actions
            </th>
          </tr>
        ...
        <tbody>
          ...
          >
            <td v-if="!auth.isStudent" class="px-4 py-3.5 pl-5">
            ...
            <td v-if="!auth.isStudent" class="px-4 py-3.5 text-[13px] text-slate-600">
              {{ r.user?.student_id != null ? '0' + r.user.student_id : '—' }}
            </td>
            <td v-if="auth.isStudent" class="px-4 py-3.5 pl-5 font-semibold text-slate-900">
              {{ startIndex + index }}
            </td>
            ...
            <td class="px-4 py-3.5 text-center">
              <span
                class="inline-flex min-w-[48px] items-center justify-center rounded-md bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600"
              >
                {{ r.duration_label ?? `${r.total_days}d` }}
              </span>
            </td>
            <td class="whitespace-nowrap px-4 py-3.5 text-[13px] text-slate-500">
              {{ r.submission_date }}
            </td>
