<template>
  <div class="rounded-[10px] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.08)]">
    <h2 class="mb-4 flex items-center gap-2 text-red-600">
      <UserX :size="18" :stroke-width="1.8" />
      Rejected Today
    </h2>

    <ActivityListSkeleton v-if="loading" :rows="3" />
    <p v-else-if="error" class="text-red-500">{{ error }}</p>
    <p v-else-if="visibleItems.length === 0" class="text-gray-400">No requests rejected today.</p>
...
function seeMore() {
  router.push({ name: 'LeaveRequests', query: { status: 'rejected' } })
}

// Page through rejected requests (newest reviewed first) in small batches
// instead of always pulling 100 records and filtering client-side. Stops
// once we have enough to render + know there's more, or once results fall
// behind today. MAX_PAGES keeps the worst case no worse than the old fetch.
const PAGE_SIZE = 20
const MAX_PAGES = 5
...
async function load() {
  ...
  try {
    for (let page = 1; page <= MAX_PAGES; page++) {
      const result = await leaveService.getLeaveRequests({ status: 'rejected', per_page: PAGE_SIZE, page })
    ...
    error.value = 'Failed to load today\'s rejections.'
  } finally {
    loading.value = false
  }
}
