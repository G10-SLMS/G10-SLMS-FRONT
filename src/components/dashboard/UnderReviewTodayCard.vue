<template>
  <div class="rounded-[10px] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.08)]">
    <h2 class="mb-4 flex items-center gap-2 text-cyan-600">
      <UserSearch :size="18" :stroke-width="1.8" />
      Under Review Today
    </h2>

    <ActivityListSkeleton v-if="loading" :rows="3" />
    <p v-else-if="error" class="text-red-500">{{ error }}</p>
    <p v-else-if="visibleItems.length === 0" class="text-gray-400">No requests marked under review today.</p>
...
async function load() {
  ...
  try {
    for (let page = 1; page <= MAX_PAGES; page++) {
      const result = await leaveService.getLeaveRequests({ status: 'under_review', per_page: PAGE_SIZE, page })
    ...
    error.value = 'Failed to load today\'s under-review requests.'
  } finally {
    loading.value = false
  }
}
